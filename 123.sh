...
readonly FIREWALL_MARKER="/etc/hy2/.firewall_managed"
...
选项：
  ...
  --no-sandbox             强制跳过需要命名空间权限的 systemd 加固项（嵌套容器用）
EOF
}
...
load_meta(){
  ...
  NS_CAPABLE=$(meta_get ns_capable "$NS_CAPABLE")
}
...
# 探测当前环境是否真的能创建 mount/uts/ipc/pid 命名空间。
# 嵌套容器（外层未开 nesting/未授予 CAP_SYS_ADMIN）通常会失败，
# 此时 ProtectSystem/PrivateTmp/PrivateDevices 等 systemd 加固项会导致服务
# 在 USER 步骤（降权前搭沙箱）直接 217/USER 退出。用真实探测代替
# "是不是容器"的猜测，避免误判（比如开了 nesting 的容器其实是支持的）。
probe_namespace_support(){
  if (( FORCE_NO_SANDBOX )); then
    NS_CAPABLE=0
    warn "已通过 --no-sandbox 强制跳过命名空间相关的 systemd 加固项"
    return 0
  fi
  if command_exists unshare && unshare --mount --uts --ipc --pid --fork -- true >/dev/null 2>&1; then
    NS_CAPABLE=1
    ok "命名空间隔离测试通过，将启用完整 systemd 沙箱"
  else
    NS_CAPABLE=0
    warn "命名空间创建受限（很可能运行在嵌套容器中，缺少 CAP_SYS_ADMIN），已自动降级 systemd 沙箱强度"
  fi
}
...
detect_firewall(){
  if command_exists ufw && ufw status 2>/dev/null | grep -q '^Status: active'; then
    echo "ufw"
  elif command_exists iptables && iptables -L -n >/dev/null 2>&1; then
    echo "iptables"
  else
    echo "none"
  fi
  ...
  case $fw_type in
    ufw)
      ...
      if ufw status | grep -Eq "${port}/udp[[:space:]]+ALLOW"; then
        ok "UFW 已存在 ${port}/udp 放行规则"
        return 0
      fi
      if (( DRY_RUN )); then
        info "[DRY-RUN] ufw allow ${port}/udp"
        return 0
      fi
      if ufw allow "${port}/udp" >/dev/null 2>&1; then
        ok "UFW 已添加 ${port}/udp 放行规则"
        echo "ufw:${port}" > "$FIREWALL_MARKER"
        return 0
      else
        warn "UFW 规则添加失败，请手动执行：sudo ufw allow ${port}/udp"
        return 1
      fi
      ;;
    iptables)
      info "检测到 iptables 防火墙，正在配置..."
      if iptables -C INPUT -p udp --dport "$port" -j ACCEPT >/dev/null 2>&1; then
        ok "iptables 已存在 ${port}/udp 放行规则"
        return 0
      fi
      if (( DRY_RUN )); then
        info "[DRY-RUN] iptables -A INPUT -p udp --dport ${port} -j ACCEPT"
        return 0
      fi
      if iptables -A INPUT -p udp --dport "$port" -j ACCEPT 2>/dev/null; then
        ok "iptables 已添加 ${port}/udp 放行规则"
        if command_exists iptables-save && command_exists netfilter-persistent; then
          netfilter-persistent save >/dev/null 2>&1 && info "iptables 规则已持久化" || warn "iptables 规则持久化失败"
        elif command_exists iptables-save && [[ -d /etc/iptables ]]; then
          iptables-save > /etc/iptables/rules.v4 2>/dev/null && info "iptables 规则已持久化" || warn "iptables 规则持久化失败"
        else
          warn "未找到 iptables 持久化工具，重启后规则将丢失"
          warn "建议安装：apt-get install iptables-persistent"
        fi
        echo "iptables:${port}" > "$FIREWALL_MARKER"
        return 0
      else
        warn "iptables 规则添加失败，请手动执行：sudo iptables -A INPUT -p udp --dport ${port} -j ACCEPT"
        return 1
      fi
      ;;
    none)
      info "未检测到活动的本地防火墙 (UFW/iptables)"
      return 0
      ;;
    *)
      warn "不支持的防火墙类型：${fw_type}"
      return 1
      ;;
  esac
}
...
remove_firewall_rule(){
  [[ -r $FIREWALL_MARKER ]] || return 0
  local marker port fw_type
  marker=$(cat "$FIREWALL_MARKER")
  fw_type=${marker%%:*}
  port=${marker##*:}
  
  case $fw_type in
    ufw)
      if confirm "是否删除 UFW 规则 ${port}/udp？(y/N): " n; then
        ufw delete allow "${port}/udp" >/dev/null 2>&1 && ok "已删除 UFW 规则" || warn "删除 UFW 规则失败"
      fi
      ;;
    iptables)
      if confirm "是否删除 iptables 规则 ${port}/udp？(y/N): " n; then
        iptables -D INPUT -p udp --dport "$port" -j ACCEPT 2>/dev/null && ok "已删除 iptables 规则" || warn "删除 iptables 规则失败"
      fi
      ;;
  esac
...
preflight_network(){
  ...
  if [[ $fw_type != "none" ]]; then
    if (( AUTO_FIREWALL )); then
      info "自动配置模式已启用"
      configure_local_firewall "$SET_PORT" || warn "本地防火墙自动配置失败"
    elif (( ! NON_INTERACTIVE )); then
      if confirm "检测到 ${fw_type} 防火墙，是否自动配置放行 ${SET_PORT}/udp？(Y/n): " y; then
        configure_local_firewall "$SET_PORT" || warn "本地防火墙配置失败"
      else
        warn "跳过本地防火墙配置，请稍后手动执行"
        if [[ $fw_type == "ufw" ]]; then
          info "手动命令：sudo ufw allow ${SET_PORT}/udp"
        elif [[ $fw_type == "iptables" ]]; then
          info "手动命令：sudo iptables -A INPUT -p udp --dport ${SET_PORT} -j ACCEPT"
        fi
      fi
    else
      info "检测到 ${fw_type}，但未启用自动配置（可使用 --auto-firewall）"
      if [[ $fw_type == "ufw" ]]; then
        warn "请手动执行：sudo ufw allow ${SET_PORT}/udp"
      elif [[ $fw_type == "iptables" ]]; then
        warn "请手动执行：sudo iptables -A INPUT -p udp --dport ${SET_PORT} -j ACCEPT"
      fi
    fi
  fi
  
  if command_exists nft && nft list ruleset >/dev/null 2>&1; then 
    info "检测到 nftables，请确认 UDP ${SET_PORT} 入站规则"
  fi
  
  warn "重要提示：如使用云服务器（腾讯云/阿里云/AWS等），还需在云控制台的安全组中放行 UDP ${SET_PORT}"
}
...
generate_unit(){
  local out=$1
  local -a mem_lines=() sandbox_lines=() cap_lines=()

  # 注意：这里用数组 + printf '%s\n' 逐行写入，不用 "a\nb" 式的字符串拼接——
  # 后者在非 quote heredoc（<<EOF）里不会被解释成真正换行，只会原样输出字面
  # 反斜杠+n，导致 GOMEMLIMIT/MemoryHigh/MemoryMax 被压成一行塞进上一个
  # Environment= 的值里，systemd 根本不认，LOW_RAM 的内存限制形同虚设。
  if [[ $RUN_MODE == LOW_RAM ]]; then
  ...
  # ProtectSystem/ProtectHome/PrivateTmp/PrivateDevices/ProtectKernel*/
  # ProtectControlGroups/ProtectClock/ProtectHostname/RestrictNamespaces 都需要
  # systemd 在降权前创建新的 mount/uts/ipc/user 命名空间，这一步需要 CAP_SYS_ADMIN。
  # 嵌套容器（外层未开 nesting）通常不下放这个能力，装了会在 USER 步骤
  # 217/USER 直接失败。probe_namespace_support 已经做过真实探测，这里按结果
  # 动态决定是否写入这批指令。
  # LockPersonality/RestrictRealtime/RestrictSUIDSGID/NoNewPrivileges/
  # SystemCallArchitectures 是 prctl/seccomp 层面的限制，不需要命名空间权限，
  # 嵌套容器里也能正常生效，所以两种模式下都保留。
  if (( NS_CAPABLE )); then
    sandbox_lines=(
    ...
    sandbox_lines=(
      "# 嵌套容器环境（命名空间创建受限，缺少 CAP_SYS_ADMIN）："
      "# 已跳过 ProtectSystem/ProtectHome/PrivateTmp/PrivateDevices/"
      "# ProtectKernel*/ProtectControlGroups/ProtectClock/ProtectHostname/RestrictNamespaces"
    )
  fi

  (( 10#$SET_PORT < 1024 )) && cap_lines=(
...
show_status(){
  ...
  [[ -r $META_FILE ]] && {
    local sandbox_desc
    [[ $(meta_get ns_capable 1) == 1 ]] && sandbox_desc="完整" || sandbox_desc="降级（嵌套容器，已跳过命名空间相关加固）"
    echo "版本：$(meta_get version unknown)；端口：$(meta_get port unknown)；模式：$(meta_get run_mode unknown)；沙箱：${sandbox_desc}"
  }
